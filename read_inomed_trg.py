...
def read_inomed_trg(DataChan, data_info):
    """Converts trigger channel voltages into integers.
    ...
    if len(sample_times) > len(DataChan):
        print('Mismatch in time array creation.')
    ...
    trg_timediff = np.insert(np.diff(DataChan_times[:,-1]), 0, 1)[:, np.newaxis]
    ...
    sample_dist = 40
    trg_paketstart = np.where(DataChan_times[:,-1]>sample_dur*sample_dist)[0]
